...
- (void)loadWithAdUnitId:(NSString *)adUnitId targetingInfo:(NSDictionary *)targetingInfo {
    if (_rewardedStatus == FLTRewardedVideoAdStatusLoaded) {
        [self show];
    } else if (_rewardedStatus != FLTRewardedVideoAdStatusLoading) {
        // 开始加载数据
        [_rewardedChannel invokeMethod:@"onRewardedVideoAdStartLoad" arguments:@{}];
